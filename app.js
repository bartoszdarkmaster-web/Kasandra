...
/* === OSINT EVENTS (rozszerzone domeny) === */
let events=[
  {source:"Reuters",title:"Fuel depot expansion near border"},
  {source:"ISW",title:"Increased troop rotation observed"},
  {source:"Reuters Health",title:"Medical logistics readiness increased"},
  {source:"WHO",title:"Civil defense preparedness review ongoing"}
];
...
function updateSystem(){
  ...
  if(snapshots.length>2){
    ...
    document.getElementById("timelineNotes").innerText =
      d>0?`CEPI wzrósł o ${d} (logistyka, medyczne)`:
      d<0?`CEPI spadł o ${Math.abs(d)} (stabilizacja)`:
      "CEPI stabilny";
  }
  updateScenarios();
}
...
function updateScenarios(){
  ...
  document.getElementById("kinetic-why").innerText=
    "Military + logistics readiness elevated.";
  document.getElementById("hybrid-why").innerText=
    "Pressure signals without mobilization.";
  document.getElementById("deescalation-why").innerText=
    "Stabilization and lack of escalation signals.";
}

/* === MAP === */
const map=L.map("mapContainer").setView([40,20],2);
L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png").addTo(map);
[[52,27,"Logistics"],[55,37,"Command"],[35,36,"Posture"]]
.forEach(p=>L.circleMarker([p[0],p[1]],{radius:8,color:"#ff4d4d"})
